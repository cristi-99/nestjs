...
let CountriesService = class CountriesService {
    ...
    async updateCountryPopulation(data) {
        const newPopulation = data.population;
        const newYear = data.year;
        const newData = { newPopulation, newYear };
        await this.CountryModel.findOne({ country: data.country_name }).then((response) => {
            const id = { _id: response.detailsOfCountry };
            console.log(id);
            this.PopulationModel.findByIdAndUpdate(id, { year: newYear, population: newPopulation }, (err, result) => {
                if (err)
                    throw new common_1.HttpException(err, common_1.HttpStatus.INTERNAL_SERVER_ERROR);
                else
                    return `Population of ${data.country_name} was edited`;
            });
        }).catch((err) => err);
    }
};
