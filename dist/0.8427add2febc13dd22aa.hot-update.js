...
let CountriesService = class CountriesService {
    ...
    async GetAllCountries() {
        let result = [];
        await this.CountryModel.find({}).then(async (response) => {
            result = response;
            await response.map(async (val) => {
                await this.PopulationModel.findById({ _id: val.detailsOfCountry }).then((res) => {
                    console.log(res);
                });
                result.push(val);
                console.log(await this.getPopulation(val.detailsOfCountry));
            });
        });
        return result;
    }
